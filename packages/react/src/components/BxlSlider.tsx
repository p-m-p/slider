...
export interface BxlSliderProps extends BxlComponentProps {
  sliderOptions: BoxSliderOptions;
}

export class BxlSlider extends Component<BxlSliderProps, {}> {
  ...
  componentDidMount() {
    if (this.el) {
      ...
      this.boxSlider.addEventListener('before', (ev) => {
        if (this.props.onBefore) this.props.onBefore.call(undefined, ev);
      });
      this.boxSlider.addEventListener('after', (ev) => {
